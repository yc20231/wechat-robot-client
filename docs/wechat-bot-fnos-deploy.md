...
机器人直接按当前请求生成，不询问 A/B；未明确指定时由 Skill 根据图片和文字意图自然完成构图。
...
Skill 不会把“电商主图”“产品主图”等说法强制解释为纯白背景，也不会擅自加入用户没有提出的限制。每个图片请求都直接按当前文字和引用图片执行；只有用户明确要求“继续、接着、沿用上一张”等表达时，才继承上一轮图片编辑上下文。
