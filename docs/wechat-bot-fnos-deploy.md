...
含糊的电商主图请求：
引用产品图片 -> @机器人 参考这张图，做一张适用于这个产品的电商主图
机器人会先询问：A. 白底商品图，适合平台上架；B. 有背景和卖点文案的电商宣传图

明确的宣传主图：
引用产品图片 -> @机器人 参考这张图，制作一张黑灰科技风、有卖点文案的电商宣传图
...
四图替换：
依次发送参考图1、参考图2、参考图3
再引用目标图片 -> @机器人 用参考图1的商品、参考图2的人物、参考图3的背景替换到目标图，保持构图、版式和文字不变
```

多图请求会固定把引用图片作为第 1 张目标图，把最近发送的 1 到 3 张图片按发送顺序作为参考图，并在编辑提示中明确要求保留第 1 张图的构图、版式、背景和文字。参考图只提供待替换商品、人物、背景或元素的外观，不得作为最终输出画布。

Skill 不会把“电商主图”“产品主图”等含糊说法自动解释为纯白背景，也不会擅自添加“不添加宣传文字”或“不添加图标”等限制。含糊请求先询问一次风格；只有用户明确选择或提出相关要求后，才会把白底、背景和文案条件写入绘图提示词。

客户端会把含糊请求中的引用目标图和原始要求作为待选择任务保存到 Redis，有效期 5 分钟。用户随后只需回复 `A`、`选择A`、`B` 或 `B，有背景和卖点文案`，客户端会自动恢复上一轮目标图片 ID 并继续调用图片 Skill，不需要再次引用图片。待选择任务按机器人、会话和发送者隔离，选择后立即删除。

多图模式只读取同一会话、同一发送者在 5 分钟内最近发送的参考图片；引用图片始终作为目标图，即使目标图早于 5 分钟，只要消息和原图仍可下载就可以继续修改。机器人生成的结果会在开启自动上传图片时持久化到 OSS，因此可以继续引用并多次修改。
...
`菜单`显示当前群可立即执行的精简功能列表，包括 AI 对话、文生图和引用图片编辑；`帮助`按当前账号身份显示跨群类型的完整示例、图片生成与编辑流程、适用条件和操作规则。旧的`业务帮助`作为`帮助`别名保留，两者在未绑定群也可使用。多图帮助会明确“先发送 1 到 3 张参考图、再引用目标图”，并提示参考图仅在同一发送者、同一会话的 5 分钟窗口内选择。
...
| 多图替换提示找不到参考图 | 先按顺序发送 1 到 3 张参考图，再在 5 分钟内引用目标图；图片必须来自同一个微信账号和同一会话 |
