...
- `菜单`与`帮助`已列出文生图、引用图片修改、机器人生成图续改和多图替换流程。
...
图片生成与编辑由内置 AI 激活 `text-to-image` Skill：普通文字请求执行文生图；引用图片时编辑引用目标图；先发送参考图再引用目标图时可执行多图替换。最多支持 1 张目标图加 3 张参考图，参考图仅从同一会话、同一发送者、触发前 5 分钟内的图片中选择；被引用的目标图不受这 5 分钟窗口限制。

引用图片问答由客户端直接从微信原图下载并以内联 base64 发送给视觉模型，不要求为了识别输入图片而先上传 OSS；机器人生成的输出图片仍按 OSS 自动上传设置持久化。
